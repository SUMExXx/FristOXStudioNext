...
function QNAElement({question, answer}: QNAElementProps) {
  ...
  return (
    <button onClick={() => setOpen(!open)} className={`relative flex flex-col w-full rounded-[40px] text-white text-[40px] transition-all duration-300 ease-in-out ${open? 'h-[120px]': 'h-[80px]'}`}>
      <div className={`z-10 flex w-full md:h-[80px] rounded-[40px] items-center justify-between md:px-20 md:py-5 md:gap-20 bg-primary text-[60px]`}>
        ...
        <div className='flex items-center justify-center outline-none'>
          <KeyboardArrowDownIcon fontSize='inherit' className={`w-[60px] h-[60px] text-[60px] text-foreground2 transition-all duration-300 ease-in-out ${open? 'rotate-0': 'rotate-180'}`}/>
        </div>
      </div>
